"""XGBoost + Lag/Trend model for stock prediction.

Identical algorithm to XGBoostPredictor but trained on the extended
feature set produced by ``add_lag_trend_features()``:
  - Lagged close ratios          (Close_Lag1/3/5_Ratio)
  - Lagged indicator snapshots   (RSI_Lag3/5, MACD_Hist_Lag3, Volume_Ratio_Lag3)
  - ADX trend strength/direction (ADX, ADX_Pos, ADX_Neg, ADX_Cross)
  - EMA golden/death cross flag  (EMA_12_26_Cross)
  - Normalised price-slope       (Trend_Slope_10d/20d)
  - Price rank in recent range   (Price_Rank_10d/20d)

Saved as ``xgboost_lag.joblib`` so it coexists with ``xgboost.joblib``.
"""
...
logger = get_logger("models.xgboost_lag")

_UNSET = object()  # sentinel — distinguishes "not passed" from explicit None


class XGBoostLagPredictor:
    """XGBoost classifier trained on base + lag/trend features."""
    ...
    def train(
        ...
        """Train on the extended (base + lag/trend) feature matrix."""
        if feature_names:
        ...
        logger.info(f"XGBoostLag trained: {best_iter} iterations{score_str}")
